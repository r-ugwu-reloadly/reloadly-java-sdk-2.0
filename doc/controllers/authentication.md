# Authentication

```java
AuthenticationController authenticationController = client.getAuthenticationController();
```

## Class Name

`AuthenticationController`


# Reloadly-Auth

```java
CompletableFuture<DynamicResponse> reloadlyAuthAsync(
    final String contentType,
    final Object body)
```

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `contentType` | `String` | Header, Required | - |
| `body` | `Object` | Body, Required | Request Payload |

## Server

`Server.AUTHENTICATION`

## Response Type

`DynamicResponse`

## Example Usage

```java
String contentType = "application/json";
Object body = com.reloadly.auth.ApiHelper.deserialize("{\"client_id\":\"CLIENT_ID_GOES_HERE\",\"client_secret\":\"CLIENT_SECRET_GOES_HERE\",\"grant_type\":\"client_credentials\",\"audience\":\"https://topups.reloadly.com\"}");

authenticationController.reloadlyAuthAsync(contentType, body).thenAccept(result -> {
    // TODO success callback handler
}).exceptionally(exception -> {
    // TODO failure callback handler
    return null;
});
```

## Example Response

```
{
  "access_token": "eyJraWQiOiIwMDA1YzFmMC0xMjQ3LTRmNmUtYjU2ZC1jM2ZkZDVmMzhhOTIiLCJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..FltioNulWc9IV78tDC96AD7CfPe9PIl1yQjso778o4A",
  "scope": "send-topups read-operators read-promotions read-topups-history read-prepaid-balance read-prepaid-commissions",
  "expires_in": 5184000,
  "token_type": "Bearer"
}
```

## Errors

| HTTP Status Code | Error Description | Exception Class |
|  --- | --- | --- |
| 401 | Access Denied | `ApiException` |

